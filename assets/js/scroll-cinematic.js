...
(function () {
  ...
  function initScrub(cfg) {
    ...
    /* ?noframes=1 in der Adresse: Diagnose-Schalter der Prüfwerkzeuge (Screenshots, Viewport-Audit) –
       ohne die Bildsequenz laden Testseiten deterministisch und schnell. */
    const noFrames = /[?&]noframes(=|&|$)/.test(location.search);
    const useFrames = frameCount > 0 && !reduceMotion && !lowData && !noFrames;
